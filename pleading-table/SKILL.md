---
name: pleading-table
description: "爭點整理狀 Word 排版技能。接收 draft-pleading 產出的爭點整理狀雙格式草稿（上半部人讀清單 + 下半部 JSON），以 python-docx + draft-pleading/assets/pleading-tmpl.docx（共用模板）直接生成帶有爭點整理表與選填聲請調查證據表的最終 .docx。只有在使用者明確要求附表格、爭點整理表、或帶表格的爭點整理狀時才觸發；若使用者僅要求撰寫一般爭點整理狀而未要求附表格，則由 draft-pleading 技能繼續處理。凡涉及爭點整理表格排版、附表格爭點整理狀 DOCX 生成時，必須使用本技能。此外，當使用者要求「高院證據清單」「高院證據清單表」「高院格式證據清單」或在高院案件用到帶「證據附卷位置」合併標頭的 7 欄證據清單表時，只要 payload 中含有 evidence_list 節點，主腳本便會自動同場產出獨立的 DOCX 附件。"
---

# Issue DOCX Builder

接收 `draft-pleading` 產出的爭點整理狀雙格式草稿（`.md`），提取最後一個 JSON 區塊，並以 `python-docx` + `draft-pleading/assets/pleading-tmpl.docx`（兩個技能共用模板）直接生成最終 `.docx`。
...
| 技能 | 是否適用 | 用途 |
|------|----------|------|
| `draft-pleading` | ✅ | 先產出爭點整理狀雙格式草稿，供本技能讀取；若未要求附表格則由此技能完成資料 |
| `pleading-table` | ✅ 本技能 | 用於附表格爭點整理狀的最終 DOCX 生成 |

> [!IMPORTANT]
> 只要涉及**爭點整理表格排版**、**聲請調查證據表**、或**附表格爭點整理狀 DOCX 生成**，就應優先使用本技能，而不是 `draft-pleading`。
...
```bash
python "<pleading-table-skill-dir>/scripts/build_issue_table.py" issue_payload.json \
  --template "<draft-pleading-skill-dir>/assets/pleading-tmpl.docx" \
...
- `--template`：模板路徑，預設為 `draft-pleading/assets/pleading-tmpl.docx`（兩個技能共用模板，已含爭點表藍圖與爭點表_* 樣式）
...
> - `numId` 由腳本從 `pleading-tmpl.docx` 本身反查 `通用_層絆1` 樣式得到
...
僅在 payload 包含 `evidence_list` 且 `items` 非空時產出，詳細規則見 `references/evidence-list-rules.md`。

- 自動在**輸出目錄**產出獨立 DOCX，不併入主書狀
- 檔名取自 `evidence_list.title`
- 表格結構：**雙層標題列**（列0+列1）+ 資料列（列2起）
...
- 標題列（列0、列1）全部雙向置中，必設 `tblHeader`
- 資料列依證據時間升冪排序，`0000000` 排在最後
- 序號由腳本自動生成，不從 payload 讀取
- 法院卷宗頁碼中之 `-`，若後方數字大於前方，自動轉換為 `~`（表示頁碼範圍）；否則保留 `-`（表示分頁編碼）
...
| 檔案 | 用途 |
|------|------|
| `../draft-pleading/assets/pleading-tmpl.docx` | 兩個技能共用模板（含書狀段落樣式、爭點表_* 樣式及三個藍圖表格）|
...
| `references/evidence-list-rules.md` | 高院證據清單表欄位規則與 AI 工作流程 |