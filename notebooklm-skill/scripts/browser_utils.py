...
class StealthUtils:
    ...
    def human_type(page: Page, selector: str, text: str, wpm_min: int = 320, wpm_max: int = 480):
        ...
        if len(text) > 200:
            # For long text, type instantly to avoid timeouts
            element.fill(text)
        else:
            # Type
            for char in text:
    ...
    def random_mouse_movement(page: Page):
        """Perform random mouse movements to simulate human inspection"""
        try:
            ...
            print(f"  ⚠️ Could not perform random mouse movement: {e}")
