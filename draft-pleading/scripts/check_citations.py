...
# 偵測各類證據編號：甲證1、乙證2、附件1...
_EVIDENCE_TAG = re.compile(
    r'（(?:(?:甲|乙|丙|丁|上|被上|原|被|附)證(?:\d+|[一二三四五六七八九十百]+)|附件(?:\d+|[一二三四五六七八九十百]+))）'
)
...
# 裁判字號識別（最高法院、高院、地院、行政法院、憲法法庭、外國法院等）
_RULING_CITE = re.compile(
    r'(?:最高法院|臺灣高等法院|高等法院|臺中高分院|高院|地方法院|臺灣.*?法院|行政法院|最高行政法院)'
    r'\s*\d{2,3}\s*年度?\s*(?:台|臺)?[\w]+字第\s*\d+\s*號'
    r'|(?:憲法法庭|司法院)\s*(?:\d{2,3}\s*年\s*憲判字第\s*\d+\s*號|釋字第\s*\d+\s*號)'
    r'|(?:德國聯邦憲法法院|日本最高裁判所|日本最高法院|美國聯邦最高法院).*?(?:判決|裁定)'
)

# 裁判純列舉（排除客體/聲明中的清單）
_RULING_LISTING = re.compile(r'^[一二三四五六七八九十]+、.*?(?:裁定|判決|決定)(?:違憲.*)?。?$')

# 引號包裹的原文（「…」或『…』，允許換行）
_QUOTED_TEXT = re.compile(r'[「『][\s\S]{1,1000}?[」』]')

# 系爭客體原文檢查
_CONST_OBJECT_TRIGGER = re.compile(
    r'(?:系爭規定|系爭判決|確定終局裁判|系爭法規範)(?:.*?)(?:規定|明定|載明|指出|認為|略以|意旨)'
)
...
def check_draft(md_path: str) -> tuple[list[dict], list[dict]]:
    ...
    for i, line in enumerate(lines):
        ...
        for m in _EVIDENCE_TAG.finditer(line):
            ...
            is_list_line = bool(re.match(r'^（?(?:[甲乙丙丁上被原附]證|附件)\d*[：:]', stripped))
        ...
        # 裁判字號後缺乏引號原文
        if _RULING_CITE.search(line) and not re.match(r'^案號[：:]', stripped) and not _RULING_LISTING.match(stripped):
            if not _QUOTED_TEXT.search(ctx):
                add_violation(
                    'RULING_NO_QUOTE', i,
                    stripped,
        ...
        # ── 規則 4-2: CONST_OBJECT_NO_QUOTE ────────────────────────────────────────
        # 引用系爭客體卻未附原文
        if _CONST_OBJECT_TRIGGER.search(line):
            if not _QUOTED_TEXT.search(ctx):
                add_violation(
                    'CONST_OBJECT_NO_QUOTE', i,
                    stripped,
                    '提及系爭規定或確定終局裁判內容時，必須以「……」引號摘錄原文，不可僅以自己的話改寫。'
                )
